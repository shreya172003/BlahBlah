...
function SelectNoteButton({ note }: Props) {
  ...
  const blankNoteTitle = "Untitled Note";
  let noteText = localNoteText ? localNoteText.split("\n")[0] : blankNoteTitle;
  if (shouldUseGlobalNoteText) {
    noteText = selectedNoteText ? selectedNoteText.split("\n")[0] : blankNoteTitle;
  }

  // Handle empty or placeholder text
  if (!noteText || noteText === "Enter note heading...") {
    noteText = blankNoteTitle;
  }
